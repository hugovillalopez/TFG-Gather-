...
export default function EditarEquipo({onClose}){
    ...
    const actualizarEquipo = async (equipo,foto) =>{
        try {
            if (foto) {
                const response = await actualizarFotoEquipo(equipo) 
                setEquipo(response)
            } else{
                console.log(Equipo._id)
                const response = await updateEquipo(Equipo._id,equipo) 
    ...
    const Validar = (e) =>{
        ...
        if (!error) {
            if (form.fotoEquipo.files[0]) {
                datos.foto = form.fotoEquipo.files[0]
                datos.idEquipo = Equipo._id
                const formData = convertirAFormData(datos)
                actualizarEquipo(formData,true)
            } else {
                actualizarEquipo(datos,false)
            }
    ...
    useEffect(() =>{
        ...
        if (Object.keys(usuarioLogueado).length != 0) {
            if (Object.keys(Equipo).length > 0 && !Equipo.admins.includes(usuarioLogueado._id)) {
                ...
                <div className="overflow-auto rounded-lg shadow-lg p-6 w-1/3 dark:bg-gray-800 bg-gray-100 h-5/6 lg:w-3/6 w-5/6 text-gray-800 dark:text-gray-300">
                    <div className="mb-5 flex flex-row items-center w-full justify-between">
                        <div className="mb-2 flex flex-row items-center">
                            <h1 className="text-xl font-bold text-black capitalize dark:text-gray-200">Editar Gather</h1>
                        </div>
                        <div className="mb-2 flex flex-row items-center">
                    ...
                    {Object.keys(errores).length != 0 && mostrarErrores &&
                        ...
                        <form ref={formRef} onSubmit={Validar} encType="multipart/form-data">
                            ...
                            <div className="flex justify-center mt-6">
                                <button className="px-10 py-3 leading-5 text-black font-bold transition-colors duration-200 transform bg-orange-500 rounded-md hover:bg-orange-400 focus:outline-none focus:bg-orange-800">Crear Equipo</button>
                            </div>
                        </form>
                    </div>
                </div>
